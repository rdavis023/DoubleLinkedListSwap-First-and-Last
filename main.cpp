...
public:
  ...
  DoublyLinkedList(int value) {
    Node *newNode =
        new Node(value); // Create a new Node object with the given value.
    head = newNode;      // Set 'head' to point to this new node.
    tail = newNode;      // Set 'tail' to also point to this new node.
    length = 1; // Initialize 'length' to 1 as there is one node in the list.
  }
  ~DoublyLinkedList() {
    Node *temp = head;
    while (head) {
